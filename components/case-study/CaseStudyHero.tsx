    ...
    <section id="top" className="border-b border-slate-200 bg-white py-14 sm:py-20">
      <div className="container-wide">
        ...
        <div className="max-w-4xl">
          ...
          <h1 className="mt-5 text-[clamp(2.25rem,8vw,4.75rem)] font-extrabold leading-[1.08] tracking-[-.055em] [overflow-wrap:anywhere]">{title}</h1>
          <p className="mt-7 max-w-3xl text-base leading-8 text-slate-600 sm:text-lg">{description}</p>
          <div className="mt-8 flex flex-wrap gap-3">
            {actions.map((action, index) => action.href ? <a key={action.label} href={action.href} target="_blank" rel="noopener noreferrer" className={index === 0 ? "button-primary" : "button-secondary"}>{action.label} <ArrowUpRight aria-hidden="true" size={18} /></a> : <span key={action.label} className="button-secondary cursor-not-allowed text-slate-400" title={`${action.label} URL coming soon`}>{action.label} · Soon</span>)}
