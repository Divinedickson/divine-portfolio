...
export default function ChatMessage({ message }: { message: ChatMessageType }) {
  ...
  return (
    <li className={`flex ${fromUser ? "justify-end" : "justify-start"}`}>
      <div className={`max-w-[88%] rounded-xl px-4 py-3 text-sm leading-6 shadow-sm ${fromUser ? "rounded-br-sm bg-blue-600 text-white" : "rounded-bl-sm border border-slate-200 bg-white text-slate-700"}`}>
