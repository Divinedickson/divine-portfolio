...
export default function ChatInput({ value, onChange, onSend, loading, inputRef }: Props) {
  useEffect(() => {
    ...
    const maxHeight = window.matchMedia("(max-width: 639px)").matches ? 112 : 128;
    input.style.height = `${Math.min(input.scrollHeight, maxHeight)}px`;
    input.style.overflowY = input.scrollHeight > maxHeight ? "auto" : "hidden";
  }, [inputRef, value]);

  return (
    <form className="shrink-0 border-t border-slate-200 bg-white px-3 pt-3 pb-[max(.75rem,env(safe-area-inset-bottom))] min-[360px]:px-4 sm:px-5 sm:pb-[max(1rem,env(safe-area-inset-bottom))]" onSubmit={(event) => { event.preventDefault(); onSend(); }}>
      ...
      <div className="flex items-end gap-2 rounded-lg border border-slate-300 bg-white p-2 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100">
        <textarea
          ...
          className="max-h-28 min-h-11 min-w-0 flex-1 resize-none overflow-y-hidden bg-transparent px-2 py-2 text-sm leading-6 text-slate-800 outline-none placeholder:text-slate-400 sm:max-h-32"
        />
